"""doe_site_group_contract

Revision ID: ab6361a582b3
Revises: ed47bdab477f
Create Date: 2026-07-27 00:00:01.000000

Part 2 of 2 (contract) of migrating DynamicOperatingEnvelope from a per-Site FK to a per-SiteGroup
FK. Requires the backfill migration (ed47bdab477f) to have already run and populated
site_group_id on every row of dynamic_operating_envelope/archive_dynamic_operating_envelope.
...
This migration:
  ...
  3. Builds the renamed/new indexes with CREATE INDEX CONCURRENTLY (outside a transaction block)
     so multi-million-row index builds don't take an ACCESS EXCLUSIVE lock for their duration.
  4. Drops the old site_id columns, their indexes, and the old FK.
"""
...
def downgrade() -> None:
    ...
    for table in (_DOE_TABLE, _ARCHIVE_DOE_TABLE):
