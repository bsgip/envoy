...
@pytest.mark.anyio
@pytest.mark.parametrize("site_id, expected_doe_count", [
    (1, 3),
    (2, 1),
...
async def test_get_derprogram_list(client: AsyncClient, uri_derp_list_format, uri_derp_doe_format, uri_derc_list_format,
                                   site_id: int, expected_doe_count: Optional[int], agg_1_headers):
...
async def get_derprogram_doe(client: AsyncClient, uri_derp_doe_format, uri_derc_list_format,
                                   site_id: int, expected_doe_count: Optional[int], agg_1_headers):
    """Being a virtual entity - we don't go too hard on validating the paging (it'll always
    be a single element or a 404)"""
