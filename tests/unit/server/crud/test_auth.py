...
async def test_select_client_ids_using_lfdi(pg_base_config):
    ...
    async with generate_async_session(pg_base_config) as session:
        ...
        assert await select_client_ids_using_lfdi(cert1_lfdi_active_agg1, session) == ClientIdDetails(certificate_id=1, aggregator_id=1)

        assert await select_client_ids_using_lfdi(cert2_lfdi_active_agg1, session) == ClientIdDetails(certificate_id=2, aggregator_id=1)

        # This is an expired cert
        assert await select_client_ids_using_lfdi(cert3_lfdi_expired_agg1, session) is None

        assert await select_client_ids_using_lfdi(cert4_lfdi_active_agg2, session) == ClientIdDetails(certificate_id=4, aggregator_id=2)
