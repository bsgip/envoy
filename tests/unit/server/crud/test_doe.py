...
def assert_doe_for_id(expected_doe_id: Optional[int],
                      expected_site_id: int,
                      expected_datetime: Optional[datetime],
                      expected_tz: Optional[str],
                      actual_rate: Optional[DOE]):
    """Asserts the supplied doe matches the expected values for a doe with that id"""
    if expected_doe_id is None:
        ...
        if expected_datetime:
            ...
            expected_in_local = datetime(expected_datetime.year,
...
@pytest.mark.anyio
async def test_select_and_count_doe_filters(pg_base_config, expected_id_and_starts: list[tuple[int, datetime]],
                                            agg_id: int, site_id: int):
...
@pytest.mark.parametrize(
    "expected_id_and_starts, agg_id, site_id",
    [
        ([(1, datetime(2022, 5, 6, 8, 2)), (2, datetime(2022, 5, 6, 10, 4)), (4, datetime(2022, 5, 7, 8, 2))], 1, 1),  # Adjusted for LA time
        ([(3, datetime(2022, 5, 6, 8, 2))], 1, 2),    # Adjusted for LA time
        ([], 2, 1),    # Adjusted for LA time
     ],
)
@pytest.mark.anyio
async def test_select_and_count_doe_filters_la_time(pg_la_timezone, expected_id_and_starts: list[tuple[int, datetime]],
                                                    agg_id: int, site_id: int):
...
@pytest.mark.anyio
async def test_select_doe_for_day_pagination(pg_base_config, expected_ids: list[int], start: int, after: datetime,
                                             limit: int):
    """Tests out the basic pagination features"""
    async with generate_async_session(pg_base_config) as session:
...
@pytest.mark.anyio
async def test_select_and_count_doe_for_day_filters(pg_base_config,
                                                    expected_id_and_starts: list[tuple[int, datetime]],
                                                    agg_id: int, site_id: int, day: date):
    """Tests out the basic filters features and validates the associated count function too"""
    async with generate_async_session(pg_base_config) as session:
...
@pytest.mark.parametrize(
    "expected_id_and_starts, agg_id, site_id, day",
    [
        ([(1, datetime(2022, 5, 6, 8, 2)), (2, datetime(2022, 5, 6, 10, 4))], 1, 1, date(2022, 5, 6)),  # Adjusted for LA time
        ([(4, datetime(2022, 5, 7, 8, 2))], 1, 1, date(2022, 5, 7)),    # Adjusted for LA time
        ([], 1, 1, date(2022, 5, 8)),    # Adjusted for LA time
     ],
)
@pytest.mark.anyio
async def test_select_and_count_doe_for_day_filters_la_time(pg_la_timezone,
                                                            expected_id_and_starts: list[tuple[int, datetime]],
                                                            agg_id: int, site_id: int,
                                                            day: date):
