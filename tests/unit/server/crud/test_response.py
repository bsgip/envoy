...
@pytest.mark.parametrize(
    ...
    [
        (1, None, 0, 99, datetime.min, 3, [3, 2, 1]),
        (1, 1, 0, 99, datetime.min, 2, [2, 1]),
        (1, 2, 0, 99, datetime.min, 1, [3]),
        (1, None, 1, 99, datetime.min, 3, [2, 1]),  # Testing skip
        (1, None, 2, 99, datetime.min, 3, [1]),  # Testing skip
        (1, None, 3, 99, datetime.min, 3, []),  # Testing skip
        (1, None, 0, 2, datetime.min, 3, [3, 2]),  # Testing limit
        (1, None, 1, 1, datetime.min, 3, [2]),  # Testing limit + skip
        (
            1,
            None,
            0,
...
async def test_select_and_count_doe_response_filters(
    ...
    async with generate_async_session(pg_base_config) as session:
        ...
        assert_list_type(DynamicOperatingEnvelopeResponse, actual_responses, len(expected_ids))
        assert [e.dynamic_operating_envelope_response_id for e in actual_responses] == expected_ids
...
async def test_select_and_count_rate_response_filters(
    ...
    async with generate_async_session(pg_base_config) as session:
        ...
        assert_list_type(TariffGeneratedRateResponse, actual_responses, len(expected_ids))
        assert [e.tariff_generated_rate_response_id for e in actual_responses] == expected_ids
