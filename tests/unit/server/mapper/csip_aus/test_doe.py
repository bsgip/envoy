...
def test_map_to_list_response():
    ...
    doe3: DynamicOperatingEnvelope = generate_class_instance(DynamicOperatingEnvelope, seed=505, optional_is_none=True, generate_relationships=False)
    doe4: DynamicOperatingEnvelope = generate_class_instance(DynamicOperatingEnvelope, seed=606, optional_is_none=True, generate_relationships=True)
    site_count = 199
    ...
    assert len(set([derc.mRID for derc in result.DERControl])) == len(all_does), f"Expected {len(all_does)} unique mrid's in the children"
